...
}:
{
  ...
  programs.ssh.knownHosts."dosvm3.cit.tum.de" = {
  ...
  systemd.services.update-authorized-keys =
    ...
    in
    {
      description = "Update authorized keys on dosvm3.cit.tum.de";
      wantedBy = [ "multi-user.target" ];
      script = ''
        ${pkgs.openssh}/bin/ssh -T -v -i $CREDENTIALS_DIRECTORY/deploy-keys deploy-keys@dosvm3.cit.tum.de < ${authorizedKeys}
      '';
      serviceConfig = {
        Type = "oneshot";
        DynamicUser = true;
