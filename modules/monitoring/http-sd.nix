...
let
  ...
  # Determine the config file path based on whether environmentFiles is used
  telegrafConfigPath =
    if config.services.telegraf.environmentFiles != [ ] then
      "/var/run/telegraf/config.toml"
    else
      # When no environmentFiles, use the generated config file directly
      # This matches what nixpkgs does internally
      (pkgs.formats.toml { }).generate "config.toml" config.services.telegraf.extraConfig;
in
{
  ...
  config = lib.mkIf (cfg.targets != { }) {
    ...
    systemd.services.telegraf.serviceConfig.ExecStart = lib.mkForce [
      ""
      "${pkgs.telegraf}/bin/telegraf -config ${telegrafConfigPath} -config-directory ${outputDir}"
    ];
  };
}
